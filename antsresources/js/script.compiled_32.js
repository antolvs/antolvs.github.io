...
if (switchEl) {
  switchEl.onclick = function () {
    ...
    if (switchOn) {
      ...
      switchEl.src = "/images/electricity_on.png";
    } else {
      letters.forEach(function (l) {
        if (fell && l.id === "logo-n") return;
      ...
      switchEl.src = "/images/electricity_off.png";
    }
  };
}
...
const lastfmUser = "antolvs";
const apiKey = "fab852d2043a8f45bd062b49feaf62d0";
...
function fetchListen() {
  fetch(
    ...
    .then(function (data) {
      ...
      // Reshape into the same object your old code expected
      const i = {
        nowplaying: nowplaying,
        name: track.name || null,
        artist: track.artist ? track.artist["#text"] : null,
        url: track.url || null,
        image: track.image && track.image[2] ? track.image[2]["#text"] : null, // large size
        playedAt: !nowplaying && track.date ? new Date(track.date.uts * 1000).toISOString() : null,
      };
      ...
      if (i.image) document.getElementById("albumcover").src = i.image;
      ...
      if (i.artist) {
        document.getElementById("artist").innerText = i.artist;
        document.getElementById("artist").textContent = i.artist;
      } else {
...
function connectChat() {
  client = new WebSocket(`${window.location.protocol === "https:" ? "wss" : "ws"}://${window.location.hostname}/services/ws`);
    ...
    setTimeout(function () {
      ...
      setTimeout(function () {
        ...
        setTimeout(function () {
          ...
          document.getElementById("webtiles-iframe").style.filter = "brightness(0.3) blur(5px)";
...
muterain.addEventListener("change", function () {
  if (muterain.checked) {
    ...
    if (window.isSnowSeason) {
      if (localStorage.disablerain != "1") snowBgm.play();
    } else {
      rainBgm.play();
...
disablerain.addEventListener("change", function () {
  if (disablerain.checked) {
    ...
    if (window.isSnowSeason) {
      ...
      if (localStorage.muterain != "1") snowBgm.play();
    } else {
      document.getElementById("snow").hidden = true;
      if (localStorage.muterain != "1") rainBgm.play();
    }
